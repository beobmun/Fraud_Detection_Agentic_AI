...
class OntologyAdmin:
    def __init__(self):
        ...
        self.added_nations = set()
        self.added_states = set()
    ...
    def insert_users_data(self, users: pd.DataFrame):
        ...
        nation_uri = self.GEO.Nation["United_States"]
        ...
        for _, user in users.iterrows():
            ...
            user_uri = self.FDA.User[user_dict['UUID']]
            self.g.add((user_uri, RDF.type, self.FDA.User))
            
            city = user_dict.get('City')
            state = user_dict.get('State')
            
            state_uri = self.GEO.State[state.replace(" ", "_")]
            city_uri = self.GEO.City[city.replace(" ", "_")]

            if state not in self.added_states:
                self.g.add((state_uri, self.GEO.partOf, nation_uri))
                self.added_states.add(state)
            if city not in self.added_cities:
                self.g.add((city_uri, RDF.type, self.GEO.City))
                self.g.add((city_uri, self.GEO.partOf, state_uri))
                self.added_cities.add(city)
    ...
    def insert_merchants_data(self, merchants: pd.DataFrame):
        ...
        for _, merchant in merchants.iterrows():
            ...
            merchant_uri = self.FDA.Merchant[merchant_dict['UUID']]
            ...
            if state == None:
                ...
                nation_uri = self.GEO.Nation[nation]
                ...
                nation_uri = self.GEO.Nation[state.replace(" ", "_")]
                if nation_uri not in self.added_nations:
                    self.g.add((nation_uri, RDF.type, self.GEO.Nation))
                    self.added_nations.add(nation_uri)
                city_uri = self.GEO.City[city.replace(" ", "_")]
            ...
            if merchant_dict.get('MCC'):
                self.g.add((merchant_uri, self.FDA.hasMCC, Literal(merchant_dict['MCC'], datatype=XSD.integer)))
            if merchant_dict.get('Name'):
                self.g.add((merchant_uri, self.FDA.hasName, Literal(merchant_dict['Name'], datatype=XSD.string)))
            
        print("Merchants data inserted into ontology successfully.", flush=True)
        return self
    
    def insert_card_data(self, data: dict):
        card_uri = self.FDA.Card[data['UUID']]
        user_uri = self.FDA.User[data['User']]

        self.g.add((card_uri, RDF.type, self.FDA.Card))

        self.g.add((user_uri, self.FDA.own, card_uri))

        if data.get('Card Brand'):
            self.g.add((card_uri, self.FDA.hasCardBrand, Literal(data['Card Brand'], datatype=XSD.string)))
        if data.get('Card Type'):
            self.g.add((card_uri, self.FDA.hasCardType, Literal(data['Card Type'], datatype=XSD.string)))
        if data.get('Expires'):
            self.g.add((card_uri, self.FDA.hasExpires, Literal(data['Expires'], datatype=XSD.dateTime)))
        if data.get('Has Chip') != None:
            self.g.add((card_uri, self.FDA.hasChip, Literal(data['Has Chip'], datatype=XSD.boolean)))
        if data.get('Cards Issued'):
            self.g.add((card_uri, self.FDA.hasCardsIssued, Literal(data['Cards Issued'], datatype=XSD.integer)))
        if data.get('Credit Limit'):
            self.g.add((card_uri, self.FDA.hasCreditLimit, Literal(data['Credit Limit'], datatype=XSD.float)))
        if data.get('Acct Open Date'):
            self.g.add((card_uri, self.FDA.hasAcctOpenDate, Literal(data['Acct Open Date'], datatype=XSD.date)))
        if data.get('Year PIN last Changed'):
            self.g.add((card_uri, self.FDA.hasYearPINLastChanged, Literal(data['Year PIN last Changed'], datatype=XSD.integer)))
        if data.get('Card on Dark Web') != None:
            self.g.add((card_uri, self.FDA.hasCardOnDarkWeb, Literal(data['Card on Dark Web'], datatype=XSD.boolean)))
        
    def insert_transaction_data(self, data: dict):
        transaction_uri = self.FDA.Transaction[data['UUID']]
        card_uri = self.FDA.Card[data['Card']]
        merchant_uri = self.FDA.Merchant[data['Merchant']]
        ...
        if data.get('Datetime') != None:
            self.g.add((transaction_uri, self.FDA.hasDatetime, Literal(data['Datetime'], datatype=XSD.dateTime)))
        if data.get('Use Chip') != None:
            self.g.add((transaction_uri, self.FDA.hasUseChip, Literal(data['Use Chip'], datatype=XSD.boolean)))
